      ...
      class CIntersection {
         ...
         public:
            ...
            static double point_segment_squared_distance(const T &c, const T &a, const T &b, char &endpoint, T &p)
            {
             ...
             /// ----------------------------------------------------------------------------
        /// @brief point_segment_squared_distance_3d
        ///
        /// same as above, but for 3D and return indication endpoint indication and the point itself
        /// endpoint indication:
        /// 's' inside segment
        /// 'a' at the a point
        /// 'b' at the b point
        /// ----------------------------------------------------------------------------
        static double point_segment_squared_distance_3d(const T &c, const T &a, const T &b, char &endpoint, T &p) {
            const double r_num = (c.x - a.x) * (b.x - a.x) + (c.y - a.y) * (b.y - a.y) + (c.z - a.z) * (b.z - a.z);
            const double r_den = (b.x - a.x) * (b.x - a.x) + (b.y - a.y) * (b.y - a.y) + (b.z - a.z) * (b.z - a.z);
            const double r = r_num / r_den;
            double ret;
            ...
            if ((r >= 0) && (r <= 1)) { //segment distance is line distance
                ...
                p.y = a.y + r * (b.y - a.y);
                p.z = a.z + r * (b.z - a.z);
//                DEBUG("between a = [" << a << "], b = [" << b << "]");
//                DEBUG("p = [" << p << "]");
                //ret = s * s / r_den;
                ret = squared_distance(c, p);
                endpoint = 's';
            } else { //segment distance is distance from c to a or from c to b
                const double dist1 = squared_distance(c, a);
                const double dist2 = squared_distance(c, b);
                if (dist1 < dist2) {
                    ret = dist1;
                    p = a;
//                    DEBUG("closest is a: a = [" << a << "], b = [" << b << "]");
//                    DEBUG("p = [" << p << "]");
                    endpoint = 'a';
                } else {
                    ret = dist2;
                    p = b;
//                    DEBUG("closest is b: a = [" << a << "], b = [" << b << "]");
//                    DEBUG("p = [" << p << "]");
                    endpoint = 'b';
                }
            }
